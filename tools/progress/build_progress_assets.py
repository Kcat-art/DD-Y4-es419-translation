...
HOSTESS_PO_PATHS = {
    "data/wdr_par_en/wdr/msg/uid006c0001.po",

    "data/wdr_par_en/wdr/msg/uid006c0009.po",
    "data/wdr_par_en/wdr/msg/uid006c000a.po",
    "data/wdr_par_en/wdr/msg/uid006c000b.po",
    "data/wdr_par_en/wdr/msg/uid006c0016.po",
    "data/wdr_par_en/wdr/msg/uid006c001d.po",
    "data/wdr_par_en/wdr/msg/uid006c001f.po",
    "data/wdr_par_en/wdr/msg/uid006c0021.po",
    "data/wdr_par_en/wdr/msg/uid016c030f.po",
    "data/wdr_par_en/wdr/msg/uid016c0310.po",
    "data/wdr_par_en/wdr/msg/uid016c0311.po",
    "data/wdr_par_en/wdr/msg/uid016c0312.po",
    "data/wdr_par_en/wdr/msg/uid016c0313.po",
    "data/wdr_par_en/wdr/msg/uid016c0314.po",
    "data/wdr_par_en/wdr/msg/uid016c0315.po",
}
...
def load_hidden_terms() -> list[str]:
    ...
    try:
        data = json.loads(
            HIDDEN_LINES_PATH.read_text(encoding="utf-8")
        )
    except Exception:
        return []
...
def classify_area(repo_relative: str) -> str | None:
    ...
    if path.startswith("data/auth/subtitle/"):
        return "Cinemáticas"
...
def load_no_translation_needed() -> set[str]:
    ...
    try:
        data = json.loads(
            TEXTURE_PROGRESS_PATH.read_text(encoding="utf-8")
        )
    except Exception:
        return set()
    ...
    return {
        normalize_texture_path(str(path))
        for path in paths
        if str(path).strip()
    }
...
def collect_texture_progress() -> dict:
    originals = (
        {
            normalize_texture_path(
                path.relative_to(TEXTURE_ORIGINAL_ROOT).as_posix()
            )
            for path in TEXTURE_ORIGINAL_ROOT.rglob("*.dds")
            if path.is_file()
        }
        if TEXTURE_ORIGINAL_ROOT.exists()
        else set()
    )
    ...
    translated = (
        {
            normalize_texture_path(
                path.relative_to(TEXTURE_TRANSLATED_ROOT).as_posix()
            )
            for path in TEXTURE_TRANSLATED_ROOT.rglob("*.dds")
            if path.is_file()
        }
        if TEXTURE_TRANSLATED_ROOT.exists()
        else set()
    )
    ...
    no_translation_needed = (
        load_no_translation_needed() & originals
    )
...
def main() -> None:
    ...
    entries_no_hostess_total = 0
    entries_no_hostess_translated = 0
    entries_no_hostess_reviewed = 0
    ...
    for po in po_files:
        total, translated, reviewed, hidden = parse_po(
            po,
            hidden_terms,
        )

        repo_relative = normalize_repo_path(
            po.relative_to(ROOT).as_posix()
        )
        files_total += 1
        entries_total += total
        ...
        if repo_relative not in HOSTESS_PO_PATHS:
            entries_no_hostess_total += total
            entries_no_hostess_translated += translated
            entries_no_hostess_reviewed += reviewed
        ...
        if area:
            ...
            if total and reviewed == total:
                data["files_reviewed"] += 1

            elif total and translated == total:
                data["files_translated"] += 1

    textures = collect_texture_progress()

    pct_translated = (
        entries_translated * 100.0 / entries_total
        if entries_total
        else 0.0
    )

    pct_reviewed = (
        entries_reviewed * 100.0 / entries_total
        if entries_total
        else 0.0
    )
    pct_translated_no_hostess = (
        entries_no_hostess_translated
        * 100.0
        / entries_no_hostess_total
        if entries_no_hostess_total
        else 0.0
    )

    pct_reviewed_no_hostess = (
        entries_no_hostess_reviewed
        * 100.0
        / entries_no_hostess_total
        if entries_no_hostess_total
        else 0.0
    ...
    summary = {
        ...
        "entries_no_hostess_total": entries_no_hostess_total,
        "entries_no_hostess_translated": entries_no_hostess_translated,
        "entries_no_hostess_reviewed": entries_no_hostess_reviewed,
        "pct_translated_no_hostess": round(
            pct_translated_no_hostess,
            2,
        ),
        "pct_reviewed_no_hostess": round(
            pct_reviewed_no_hostess,
            2,
        ),
        ...
        "textures_total": textures["total"],
        "textures_translated": textures["translated"],
        "textures_no_translation_needed": (
            textures["no_translation_needed"]
        ),
        "textures_completed": textures["completed"],
        "textures_pending": textures["pending"],
        "pct_textures": textures["pct"],
        "pct_global": round(
            global_pct(
                entries_translated,
                entries_reviewed,
    ...
    for filename, label, pct in (
        (
            ...
            "traducción",
            summary["pct_translated"],
        ),
        (
            "review_badge.json",
            "revisión",
            summary["pct_reviewed"],
        ),
        (
            "texture_badge.json",
            "texturas",
            summary["pct_textures"],
        ),
        (
            "global_badge.json",
            "progreso global",
    ...
    (OUT_DIR / "readme_progress.md").write_text(
        build_readme(
            summary,
            areas_out,
        ),
