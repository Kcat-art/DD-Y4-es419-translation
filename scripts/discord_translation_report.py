...
def is_entry_reviewed(
    block: str,
    file_reviewed: bool,
) -> bool:
    line_statuses = LINE_STATUS_RE.findall(
        block
    )
...
def parse_po_text(
    ...
    file_statuses = FILE_STATUS_RE.findall(
        text
    )
    ...
    for block in iter_po_blocks(text):
        ...
        msgctxt = extract_po_field(
            block,
            "msgctxt",
        )

        msgid = extract_po_field(
            block,
            "msgid",
        )
...
def count_commit_progress(
    ...
    for (
        old_path,
        new_path,
    ) in changed_po_file_pairs(
        old_commit,
        new_commit,
    ):
        old_entries = parse_po_text(
            file_at_commit(
                old_commit,
                old_path,
            ),
        ...
        for (
            ...
            old_translated = is_translated(
                old_entry
            )

            new_translated = is_translated(
                new_entry
            )

            old_reviewed = is_reviewed(
                old_entry
            )

            new_reviewed = is_reviewed(
                new_entry
            )
    ...
    texture_files = changed_texture_files(
        old_commit,
        new_commit,
    )
...
def commit_author(
    ...
    username = GITHUB_USERNAME_MAP.get(
        commit
    )
...
def build_title(
    ...
    if len(users) == 2:
        return (
            f"{users[0]} y {users[1]} "
            f"hicieron cambios en {repo_name}"
        )

    return (
        f"{len(users)} usuarios hicieron "
        f"cambios en {repo_name}"
    )


def build_user_value(
    stats: dict,
    summary: dict,
) -> str:
    parts: list[str] = []

    entries_total = as_int(
        summary.get(
            "entries_total",
            0,
        )
    ...
    if translated > 0:
        ...
        parts.append(
            f"Tradujo **{translated}** "
            f"{plural(translated, 'línea')} "
            f"en **{translated_files}** "
            f"{plural(translated_files, 'archivo')}\n"
            f"Traducción global: "
    ...
    if reviewed > 0:
        ...
        parts.append(
            f"Revisó **{reviewed}** "
            f"{plural(reviewed, 'línea')} "
            f"en **{reviewed_files}** "
            f"{plural(reviewed_files, 'archivo')}\n"
            f"Revisión global: "
    ...
    if edited > 0:
        ...
        parts.append(
            f"Ajustó **{edited}** "
            f"traducciones existentes "
            f"en **{edited_files}** "
            f"{plural(edited_files, 'archivo')}"
        )
...
def build_payload(
    ...
    entries_reviewed = as_int(
        summary.get(
            "entries_reviewed",
            0,
        )
    )
    entries_no_hostess_total = as_int(
        summary.get(
    ...
    entries_no_hostess_translated = as_int(
        summary.get(
    ...
    entries_no_hostess_reviewed = as_int(
        summary.get(
    ...
    pct_translated_no_hostess = as_float(
        summary.get(
    ...
    pct_reviewed_no_hostess = as_float(
        summary.get(
    ...
    description = (
        ...
        f"**Traducción (no hostess):** "
        f"**{format_pct(pct_translated_no_hostess)}%** "
        f"({entries_no_hostess_translated}/"
        f"{entries_no_hostess_total})\n"

        f"**Revisión (no hostess):** "
        f"**{format_pct(pct_reviewed_no_hostess)}%** "
        f"({entries_no_hostess_reviewed}/"
        f"{entries_no_hostess_total})\n"

        f"**Texturas:** "
        f"**{format_pct(pct_textures)}%** "
...
def main() -> None:
    ...
    repo_full = os.environ.get(
        "GITHUB_REPOSITORY",
        "Kcat-art/DD-Y4-es419-translation",
    )
    ...
    if payload is None:
        print(
            "No hubo líneas traducidas, "
            "revisadas, ajustadas ni "
            "texturas para reportar."
        )
        return

    card_path = (
        ROOT / "progress_card.png"
    )
